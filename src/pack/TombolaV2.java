...
public class TombolaV2 {
	...
	static int[][] M1 = null;// [[Numeri casuali cartella1], [Numeri casuali cartella2] ....
	static int[][] M2 = null;
	static int[][] M3 = null;
	static int[][] M4 = null;
	static int[][] M5 = null;
	static int[] M1Contatore = null;// [cartella1,cartella 2,...
	static int[] M2Contatore = null;
	static int[] M3Contatore = null;
	static int[] M4Contatore = null;
	static int[] M5Contatore = null;
	...
	public static void main(String[] args) {
		...
		System.out.println("Che modalità vuoi utilizzare:(manuale/automatico): ");
		String scelta=new Scanner(System.in).next();
		...
		while (!Tombola && !uscita) {// for infinito
			
			if (scelta.equals("automatico")) {
			...
			for (int i = 0; i <= NumGiocatori - 1; i++) {
				...
				while (!uscita) {
					...
					case 1:
						M1Contatore = VerificaVincete2(var, M1, M1Contatore);
						break;
					case 2:
						M1Contatore = VerificaVincete2(var, M1, M1Contatore);
						M2Contatore = VerificaVincete2(var, M2, M2Contatore);
						break;
					case 3:
						M1Contatore = VerificaVincete2(var, M1, M1Contatore);
						M2Contatore = VerificaVincete2(var, M2, M2Contatore);
						M3Contatore = VerificaVincete2(var, M3, M3Contatore);
						break;
					case 4:
						M1Contatore = VerificaVincete2(var, M1, M1Contatore);
						M2Contatore = VerificaVincete2(var, M2, M2Contatore);
						M3Contatore = VerificaVincete2(var, M3, M3Contatore);
						M4Contatore = VerificaVincete2(var, M4, M4Contatore);
						break;
					case 5:
						M1Contatore = VerificaVincete2(var, M1, M1Contatore);
						M2Contatore = VerificaVincete2(var, M2, M2Contatore);
						M3Contatore = VerificaVincete2(var, M3, M3Contatore);
						M4Contatore = VerificaVincete2(var, M4, M4Contatore);
						M5Contatore = VerificaVincete2(var, M5, M5Contatore);
						break;
					...
					System.out.println("Vuoi stampare le cartelle?(si/no): ");
					String stampa=new Scanner(System.in).next();
					if (stampa.equals("si")) {
						for (int i = 0; i < NomiGiocatori.length; i++) {
							System.out.println("Stampo cartelle di "+NomiGiocatori[i]+" : ");
							StampaCartella(i);
						}
					}
					
					
					System.out.println("Vuoi generare un'altro numero?(si/no): ");
					String decisione=new Scanner(System.in).next();
	...
	public static int[] VerificaVincete2(int Num, int[][] m, int[] cont) {// NrTombola, Mcartelle,contatoreVincete
	...
	public static void StampaCartella(int indice) {
		switch (indice) {
		case 0:
